...
Accounts.prototype.signTransaction = function signTransaction(
    ...
    if (
        isExist(tx.common) &&
        isExist(tx.common.customChain.chainId) &&
        isExist(tx.chainId) &&
        tx.chainId !== tx.common.customChain.chainId
    ) {
        error = new Error(
    ...
    function signed(tx) {
        ...
        try {
            ...
            var ethTx = TransactionFactory.fromTxData(
                transaction,
                transactionOptions
            );
            ...
            var result = {
                messageHash:
                    "0x" +
                    Buffer.from(signedTx.getMessageToSign(true)).toString(
                        "hex"
                    ),
    ...
    return Promise.all([
        isNot(tx.common) || isNot(tx.common.customChain.chainId) //tx.common.customChain.chainId is not optional inside tx.common if tx.common is provided
            ? isNot(tx.chainId)
                ? _this._ethereumCall.getChainId()
                : tx.chainId
            : undefined,
        isNot(tx.nonce)
        ...
        if (
            (isNot(txchainId) &&
                isNot(tx.common) &&
                isNot(tx.common.customChain.chainId)) ||
            isNot(txnonce) ||
            isNot(txnetworkId) ||
            isNot(txgasInfo)
        ) {
        ...
        return signed({
            ...
            ...(isNot(tx.common) || isNot(tx.common.customChain.chainId)
                ? { chainId: txchainId }
                : {}), // if common.customChain.chainId is provided no need to add tx.chainId
            nonce: txnonce,
...
function _handleTxType(tx) {
    ...
    if (
        ...
        (tx.common &&
            tx.common.hardfork &&
            tx.common.hardfork.toLowerCase() === HardForks.London) ||
        ...
        (tx.common &&
            tx.common.hardfork &&
            tx.common.hardfork.toLowerCase() === HardForks.Berlin) ||
                    ...
                    if (tx.type === "0x2" && block && block.baseFeePerGas) {
                        ...
                        if (tx.gasPrice) {
                            ...
                            maxPriorityFeePerGas =
                                tx.maxPriorityFeePerGas || "0x9502F900"; // 2.5 Gwei
                            maxFeePerGas =
                                tx.maxFeePerGas ||
                                utils.toHex(
                                    utils
...
/* jshint ignore:start */
Accounts.prototype.recoverTransaction = function recoverTransaction(
    rawTx,
    txOptions = {}
) {
...
Accounts.prototype.decrypt = function (v3Keystore, password, nonStrict) {
    ...
    var json =
        !!v3Keystore && typeof v3Keystore === "object"
            ? v3Keystore
    ...
    var mac = utils
        .sha3(Buffer.from([...derivedKey.slice(16, 32), ...ciphertext]))
        .replace("0x", "");
